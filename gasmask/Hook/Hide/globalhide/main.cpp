...
BOOL InjectByDll(HANDLE hProcess, LPCTSTR szDllName){
    HANDLE hThread;
    LPVOID pRemoteBuf;
    DWORD dwBufSize = lstrlen(szDllName) +1;
    FARPROC pfLoadLib;

    //자식프로세스에 공간 할당하고 주소 반환받음
    ...
    pfLoadLib = GetProcessAddress(GetModuleHandle("kernel32.dll"),
                                     "LoadLibraryA");

    hThread = CreateRemoteThread(hProcess, NULL, 0,
                                 (LPTHREAD_START_ROUTINE)pfLoadLib,
                                 pRemoteBuf, 0, NULL);
